...
import {
  getPayrollBillingApprovedRowsAction,
  getRateManagerDataAction,
  updatePatientServiceContractBillRateAction,
  updatePayRateScheduleRateAction,
...
export default function PayrollBillingReportContent({
  ...
  const reload = useCallback(() => {
    ...
    startTransition(async () => {
      const res = await getPayrollBillingApprovedRowsAction(dateFrom, dateTo)
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Hours breakdown per caregiver and client with pay and bill amounts for approved visits.
        </p>
      </div>
        ...
        <div className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-600 mt-0.5">{summary.visitCount} approved visits</div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="flex flex-col gap-2 border-b border-gray-100 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">Approved visits — {formatRangeLabel(dateFrom, dateTo)}</p>
            </div>
            <button
              type="button"
...
function RateManagerModal({
  ...
  const save = () => {
    ...
    startTransition(async () => {
      for (const r of payRows) {
        ...
        const res = await updatePayRateScheduleRateAction(r.id, Number(d))
        if (res.error) {
  ...
  return (
    <Modal
      ...
      subtitle="View and update caregiver pay rates and client bill rates. Changes affect all pending (unapproved) visits — historical approved records are never modified."
      size="xl"
    >
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-blue-100 bg-blue-50/80 px-4 py-3 text-sm text-blue-900">
          ...
          Rates shown are current rates applied when a pending visit is approved. Approved (locked) records retain
          their original snapshot rates permanently. Editing rates here also saves back to the associated profile.
        </div>
