-- HomeSight schema backbone - updated
-- Assumptions:
-- 1) auth.uid() = user_profiles.id
-- 2) Existing tables remain in place during migration:
--    agencies, user_profiles, staff_members, care_coordinators, patients, licensing_experts
-- 3) This script creates NEW backbone tables only. It does not rename or drop legacy tables.
